...
**TODO**
1. `http_conn::deal_timer_close_connection()`中断开客户端连接数需要立即删除定时器链表上的对应节点。在Reoactor模式下这个过程应当由工作线程完成，然而定时器链表不是线程安全的，会产生Segment fault，所以目前禁用了，断开连接的无效定时器会被主线程的定时任务当作非活动连接清理；在Proactor模式下这个过程是主线程做的，正常工作。
2. 定时器应该改成堆维护的，使用有序链表维护效率不高。
